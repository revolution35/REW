...
function RefundSummary({ info: { account, lotteryContract } }: { info: RefundTransactionInfo }) {
  // const { ENSName } = useENSName()
  return typeof lotteryContract === 'string' ? (
    <Trans>
      Refund for{' '}
      {account}
    </Trans>
  ) : (
    <Trans>Refund  for {account}</Trans>
  )
}

function BuyTicketsSummary({ info: {totalAmount, ticketsAmount, lotteryName, lotteryNonce, } }: { info: BuyTicketsTransactionInfo }) {
  // const { ENSName } = useENSName()
  return typeof totalAmount === 'string' ? (
    <Trans>
      Buy{' '}
      {"Покупка " + totalAmount + " REW, на " + ticketsAmount + " " + lotteryNonce  } 
    </Trans>
  ) : (
    <Trans>Buy  {" " + ticketsAmount + " Tickets"} </Trans>
  )
}
...
export function TransactionSummary({ info }: { info: TransactionInfo }) {
  switch (info.type) {
   ...
   case TransactionType.REFUND:
      return <RefundSummary info={info} />

   case TransactionType.BUYTICKETS:
      return <BuyTicketsSummary info={info} />
